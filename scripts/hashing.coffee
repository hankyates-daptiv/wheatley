...
module.exports = (robot) ->
  robot.respond /md5( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'md5')

  robot.respond /SHA( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'sha')

  robot.respond /SHA1( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'sha1')

  robot.respond /SHA256( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'sha256')

  robot.respond /SHA512( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'sha512')

  robot.respond /RMD160( me)? (.*)/i, (msg) ->
    msg.reply hexDigest(msg.match[2], 'rmd160')
