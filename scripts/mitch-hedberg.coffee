  ...
  robot.respond /get( dirty)? mitch$/i, (msg) ->
    msg
      ...
      .get() (err, res, body) ->
        ...
        quote = get_quote msg, err, quotes
      
get_quote = (msg, err, quotes) ->
  ...
  if pottyParm == "dirty"
    msg.reply if err then get_static_quote else quote
  else
    keep_it_clean msg, quote, (cleantext, err) ->
      cleaned = if err then get_static_quote() else cleantext 
      msg.reply cleaned

# Helpers   
get_static_quote = () ->
  quote = StaticQuotes[Math.floor(Math.random() * StaticQuotes.length)]
  "Well, I couldn't actually get a quote, so, umm, I'll just tell you one that I remember instead.\n#{quote}"

parse_html = (html, selector) ->
  handler = new HtmlParser.DefaultHandler((() ->), ignoreWhitespace: true)
  parser  = new HtmlParser.Parser handler
  parser.parseComplete html
  quotes = Select handler.dom, selector
  quotes.filter (v) ->
    not v.attribs
...
keep_it_clean = (msg, quote, cb) -> 
  msg.http("http://www.purgomalum.com/service/json")
    .query(text: quote)
    .get() (err, res, body) -> 
