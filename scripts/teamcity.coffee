...
#
# Commands:
#   hubot tc status <project> - Shows the green-ness of a project. Default is Ppm Master.
...
module.exports = (robot) ->
  ...
  projectStatus = (msg, project, callback) ->
    failedBuilds = []
    isGreen = true
    processed = 0
    getBuildTypes msg, project, (err,msg,buildTypes) ->
      _.each buildTypes, (bt,i) ->
        url = "#{base_url}/httpAuth/app/rest/buildTypes/id:#{bt.id}/builds?count=1"
        msg.http(url).headers(getAuthHeader()).get() (err, res, body) ->
          processed = processed + 1
          err = body unless res.statusCode == 200
          response = JSON.parse(body)
          if response.build.length > 0
            green = response.build[0].status == 'SUCCESS'
            isGreen = green && isGreen
            if !green
              failedBuilds.push("#{bt.name}: #{bt.webUrl}")
            if processed == buildTypes.length - 1
              status = '_RED_'
              if isGreen
                status = 'GREEN'
              callback err, msg, {status: status, redBuilds: failedBuilds}
  ...
  robot.respond /tc status ?(.*)?/i, (msg) ->
    project = "Ppm Master"
    if msg.match[1]
      project = msg.match[1]
    projectStatus msg, project, (err, msg, projectStatus) ->
      myMsg = "#{project} is #{projectStatus.status}"
      if projectStatus.redBuilds.length > 0
         myMsg = myMsg + "\nRed Builds:\n#{projectStatus.redBuilds.join('\n')}"
      msg.reply myMsg
